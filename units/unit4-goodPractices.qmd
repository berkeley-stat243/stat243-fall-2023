...
you expect. They're primarily intended for use during the development
process to provide "sanity checks" that specific conditions are true.
One would generally only expect those conditions not to be true if there is a bug
in the code. Here's an example of
...
You've already seen exceptions in action whenever you've done something
...
when the syntax is correct but the execution code results in some sort of error.

In developing your own code you can trigger an error (i.e., throw an exception)
to handle run-time errors and unusual situations that occur when your code
is being run.
