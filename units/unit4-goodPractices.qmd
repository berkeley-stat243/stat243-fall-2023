...
Exceptions can be a valuable tool for making your code handle different modes of failure (missing file, URL unreachable, premission denied, invalid inputs, etc.), you use it when you are writing code that is supposed to perform a task (e.g. a function that copies lined in a file in reverse order) to indicate that you failed and the reason for that failure (e.g. the file was not there in the first place). In  such case, you want your code to raise an exception and make it as informative as possible, typically by handling the exception thrown by another function you call and augmenting the message.
...
Some standard ways to handle exceptions are with `try-except` blocks and with `raise`.

With `try-except`, you attempt to run some code in the `try` block and
handle errors in the `except` block.

In the following example, you could let Python
just catch the error, but it can be better in terms of the experience of those
using your code for your code to handle it directly in many cases where the
error reported by Python might be hard for a user to interpret.
...
try:
    ...
    print(f"File `{fn}` not found in the current path: `{os.getcwd()}`.")
```

Here's another example. In this case,
by handling the error in our own code, we avoid a long confusing traceback message.

```{python}
#| error: true
some_bad_url = "http://missingurl.com"
try:
    requests.get("http://missingurl.com")
except Exception as err:
    print(f"There was a problem accessing {some_bad_url}. Perhaps it doesn't exist or the URL has a typo?")
```

With `raise` you can invoke an exception. Suppose we need an input to be a string.
We'll use Python's built-in `TypeError`, one of the various [exception types
...
Assertions are a quick way to raise a specific type of Exception (AssertionError). Assertions useful for performing quick checks in your code that the state of the program is as you expect. They're primarily intended for use during the development
process to provide "sanity checks" that specific conditions are true, and there are ways to disable them when you are running your code for production purposes (to improve performance). A common use is to use them for verifying preconditions and postconditions (especially preconditions). One would generally only expect such conditions not to be true if there is a bug in the code. Here's an example of
