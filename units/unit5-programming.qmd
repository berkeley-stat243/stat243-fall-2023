...
When you are searhing for all occurrrences of a pattern in a large text object, it may be beneficial to use `finditer`:
...
This method bahaves lazily and returns an iterator that gives us one match at a time, and only scans for the next match when we ask for it. This is similar to the behavior we saw with `pandas.read_csv(chunksize = n)`
...
def myfun(scalar_x, list_x, list_x_reassigned, nparray_x,
    ...
    scalar_x = 99                # global input unaffected
    list_x[0] = 99               # global input MODIFIED
    list_x_reassigned = [99,2,3] # global input unaffected
    nparray_x[0] = 99            # global input MODIFIED
...
3.  Where is `myfun` defined (this is tricky)?
...
**[TODO: is there a way to access variables in a closure (i.e., in
this case access 'y' based on `myfun`). Is there a way to find the
enclosing scope for 'myfun' and look in it? See the `inspect` package]**
...
because of numpy's row-major ordering (by default, matrices are stored in memory as a long array in which values in a row are adjacent to each other). However how numpy handles this is [more complicated](https://numpy.org/doc/stable/dev/internals.html#multidimensional-array-indexing-order-issues) (see more in Section 4.6 on the cache), such that it may not matter for numpy calculations.
