...
a name accessible in the scope in which it was imported (i.e., the local context). The mapping of
...
So `range` and `mymod` are in the global namespace and `range` and `x` are in the module namespace
of `mymod`.
...
```{python}
import mypkg.mysubmod     ## Note that __init__.py is invoked
mypkg.mysubmod.b
```
...
`import mypkg.mysubmod` to `mypkg/__init__.py`.

One would generally not import the items from `mysubmod` directly into the `mypkg` namespace
