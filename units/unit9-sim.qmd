...
By default (using `numpy.random.default_rng()`) numpy uses something called the [PCG-64 generator](https://numpy.org/doc/stable/reference/random/bit_generators/pcg64.html#numpy.random.PCG64).
As discussed in the [PCG documentation](https://www.pcg-random.org/), it uses a linear congruential generator, which we've just seen.
These generators are not great, but PCG then combines that with a [technique called *permutation functions*](https://www.pcg-random.org/pdf/hmc-cs-2014-0905.pdf) as the output function that translates the
underlying sequence to the random number output.
...
advancing an arbitrary number of steps (useful for parallelization, as well
as $2^{127}$ streams. The state of the PCG-64 RNG is represented by two
128-bit unsigned integers. While the *default* is PCG-64, simply
using the functions available via `np.random` to generate random numbers
seems to actually use the Mersenne Twister, so the meaning if *default*
seems odd here.
...
I'm not sure how `saved_state` corresponds to the state as we
saw when manually implementing linear congruential generators earlier.
...
as generated by the RNG algorithm. I'm not sure about the situation
in Python, though for the Mersenne Twister, it may be the same.
