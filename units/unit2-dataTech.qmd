...
1.  In general, you don't want to hard-code absolute paths into your
    ...
    ```{python, relative-paths, eval=FALSE}
...
3.  Using UNIX style directory separators will work in Windows, Mac or
    ...
    ```{python, path-separators, eval=FALSE}
...
import time
t0 = time.time()
dat = pandas.read_csv(os.path.join(dir, 'airline.csv'))
t1 = time.time()
dat2 = polars.read_csv(os.path.join(dir, 'airline.csv'), null_values = ['NA'])
t2 = time.time()
...
```{python, headlines, eval=FALSE}
URL = "https://www.nytimes.com"
responseNYT = requests.get(URL)
soupNYT = BeautifulSoup(responseNYT.content, 'html.parser')
...
```{python, unicode-example, eval=TRUE}
