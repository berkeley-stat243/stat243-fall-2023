...
**[Fernando, could you check that this looks ok?]**
...
- vectorized calls rather than loops:
   - vectorized calls generally run loops in compiled C code rather than having the loop run in R, and
...
Unfortunately seeing the source code in Python often involves going and finding it,
...
With a bit more digging around we could verify that `trtrs` is a LAPACK funcion by finding this result using `grep:
...
Sometimes we can exploit vectorized mathematical operatinos in
suprising ways, though sometimes the
code is uglier. 
...
We can discuss what has to happen (in terms of calculations, memory allocation, and copying) in the two vectorized approaches to try to understand which is more efficient.
...
because of Python's row-major ordering (matrices are stored in memory as a long array in which values in a row are adjacent to each other). However how numpy handles this is [more complicated](https://numpy.org/doc/stable/dev/internals.html#multidimensional-array-indexing-order-issues) (see more in Section 4.6 on the cache).
...
vector to scale the columns relative to each other, can be dangerous as the code is not transparent
...
Whether looping and map operations are faster will depend in part on whether a substantial part of the work is in the overhead involved in the looping or in the time required by the function
...
Here's an example where `map` is not faster than a loop. 
...
Given the extra computation involved in actually multiplying each number by one, it's surprising that this is faster than numpy sum function. One thing we'd want to know is where the BLAS matrix multiplication call is being done in parallel.
...
How is it that Python can look up by key in the dictionary at the same speed as jumping to an index position? It uses hashing, which allows O(1) lookup. In contrast, if one has to look through each key in turn, that is O(n), which is much slower:
...
How does this inform one's programming? For example, if you have a matrix of values stored in row-major order, computing on a column will be a lot faster than computing on a column, because the row can be read into the cache from main memory and then accessed in the cache. In contrast, if the matrix is large and therefore won't fit in the cache, when you access the values of a column, you'll have to go to main memory repeatedly to get the values for the row because the values are not stored contiguously.
...
The analogous code in R would show a substantial difference, so in this case numpy is more sophisticated thaan R.
...
**[Fernando - do you have any ideas?]**
...
[UNDER CONSTRUCTION: these need to be converted from R to Python]
